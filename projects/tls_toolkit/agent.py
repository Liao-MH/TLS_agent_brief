...
from .utils import ensure_dir, now_run_id, write_json, read_json, Timer, SimpleLogger
...
from .postprocess import postprocess_tls_gc_proximity
...
def run_pipeline_once(
    ...
    logger.log(f"[WSI] backend={wsi_info['backend']} size={wsi_info['width']}x{wsi_info['height']} levels={wsi_info['level_count']}")

    mask_level = int(params.get("mask_level", 0))
    ...
    with Timer("tissue") as t:
        tissue_mask, base_to_thumb_ds, thumb = backend.build_tissue_mask(max_thumb=int(params["max_thumb"]))
    ...
    with Timer("patch_plan") as t:
        patches = plan_patches(
            ...
            tissue_thresh=float(params["tissue_thresh"]),
            out_dir=run_dir,
        )
    ...
    with Timer("inference") as t:
        pred_by_idx, infer_stats = infer_tiles(
            ...
            level=mask_level,
            out_dir=run_dir,
            save_all_patches=bool(params.get("save_all_patches", False)),
            qc_tile_samples=int(params.get("qc_tile_samples", 12)),
    ...
    # Hard guard: if nothing was successfully inferred, stop early.
    ...
    with Timer("postprocess") as t:
        ...
        pp = postprocess_tls_gc_proximity(
            full_mask=full_mask,
            tls_label=1,
            gc_label=2,
            immune_label=3,
            min_object_area=int(params["min_object_area"]),
            proximity_px=int(params.get("tls_gc_proximity_px", 32)),
        )
    ...
    logger.log(f"[POST] converted_tls_components={postproc_stats.get('tls_components_converted', 0)}")
    # QC tile_samples AFTER postprocess (so ImmuneCluster exists)
    # Uses FINAL mask (pp.mask) to select balanced tiles and write images/pred/merge.
    try:
        qc_tile_samples = int(params.get("qc_tile_samples", 12))
        ...
        # QC evidence is non-fatal; log and continue
    ...
    with Timer("vectorize") as t:
        vz = vectorize_mask_to_geojson(
            ...
            out_dir=run_dir,
            min_object_area=int(params["min_object_area"]),
            simplify_eps=float(params.get("simplify_eps", 2.0)),
...
def run_agent(
    ...
    # Determine starting preset and effective rerun policy
    if preset_requested:
        preset_name = preset_requested
        # When a preset is explicitly requested, fallback is disabled by default.
        # Only when allow_preset_fallback=True and max_reruns>0, we allow preset reselection.
    ...
    for attempt in range(0, max_reruns_effective + 1):
        try:
            ...
            # If a preset was explicitly requested and fallback is not enabled, do not reselect presets.
