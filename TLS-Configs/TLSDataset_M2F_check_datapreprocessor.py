custom_imports = dict(
    imports=['projects.morph_inputfusion.data_preprocessor'],
...
data_preprocessor = dict(
    ...
    # 完全沿用旧 SegDataPreProcessor 的设置
    bgr_to_rgb=True,
    pad_val=0,
    seg_pad_val=255,
    size=(1200, 1200),

    # 显式写成 RGB 的 mean/std，避免和 morph 混淆
    rgb_mean=[123.675, 116.28, 103.53],
    rgb_std=[58.395, 57.12, 57.375],

    # morph 相关参数：本实验中关闭 morph，只是占位
    morph_mean=0.5,
    morph_std=0.5,
    morph_enabled=False,
)
...
model = dict(
    backbone=dict(
        ...
        init_cfg=dict(
            checkpoint=
            '/lustre1/g/path_dwhho/new_LMH/mmsegmentation/work_dirs/TLSDataset-Mask2Former/best_mIoU_iter_7000.pth',
            type='Pretrained'),
        mlp_ratio=4,
        num_heads=[
            6,
...
pretrained = '/lustre1/g/path_dwhho/new_LMH/mmsegmentation/work_dirs/TLSDataset-Mask2Former/best_mIoU_iter_7000.pth'
...
train_cfg = dict(max_iters=20000, type='IterBasedTrainLoop', val_interval=200)
