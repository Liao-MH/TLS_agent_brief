...
#SBATCH --time=2-01:00:00             # 7. Job execution duration limit day-hour:min:sec
#SBATCH --mem=50G                  # 内存总量
#SBATCH --output=logs/Best_morph.out  # 输出日志路径
#SBATCH --error=logs/Best_morph.err   # 错误日志路径（补充缺失项）
...
bash "${MMSEG_ROOT}/tools/dist_train.sh" "${CONFIG_ROOT}/Standard_config_with_morph.py" 4
